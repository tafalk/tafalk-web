      ...
      <v-row no-gutters>
        <v-col cols="12" md="8" offset-md="2">
          <v-card flat>
                    ...
                    <v-col cols="12">
                      ...
                      <v-avatar pt-1 size="150">
                        <v-img
                          v-if="
                            authenticatedUser &&
                              visitedUser.profilePictureObjectUrl
                          "
                  ...
                  <v-row>
                    <v-col cols="12">
                      ...
                      <v-btn
                        ...
                        @click.stop="
                          setIsChangeProfilePictureDialogVisible(true)
                        "
                        >{{
                          $t('user.profilePage.changeProfilePictureButtonText')
                        }}</v-btn
                      >
                    </v-col>
                  </v-row>
                  ...
                  <v-row align="center">
                    <v-col cols="12">
                      ...
                      <v-row align="center">
                        <v-col cols="12">
                          <div
                            class="display-1 text-xs-center text-sm-left grey--text"
                          >
                      ...
                      <v-row align="center" v-if="isProfileAllowed">
                        <v-col cols="12">
                          <div class="text-left grey--text">
                            <v-icon color="grey">mdi-bio</v-icon>&nbsp;{{
                              visitedUserBio
                            }}
                          </div>
                        </v-col>
                      ...
                      <v-row align="center" v-if="isProfileAllowed">
                        ...
                        <v-col cols="12" sm="6">
                          <p
                            v-if="visitedUserAccountCreationDateStr"
            ...
            <!-- Tabs -->
            <tafalk-profile-tabs
              v-if="visitedUser && isProfileAllowed"
...
export default {
  ...
  computed: {
    ...
    isVisitingOwnProfile() {
      return (
        ...
        this.visitedUser &&
        this.authenticatedUserName === this.visitedUser.username
      )
    },
    isVisitorAllowed() {
    ...
    visitedUserBio() {
      return this.visitedUser &&
        this.visitedUser.bio &&
        this.visitedUser.bio.length > 0
        ? this.visitedUser.bio
        : this.defaultBio
    },
    visitedUserColor() {
    ...
    visitedUserAccountCreationDateStr() {
      return this.visitedUser
        ? new Date(this.visitedUser.createdAt).toISOString().slice(0, 10)
        : null
    }
  },
  watch: {
  ...
  methods: {
    ...
    async getInitialInfo(username) {
      ...
      try {
        const graphqlVisitedProfileResult = await API.graphql(
        ...
        if (!visitedUserProfile) {
          ...
          const visitedUserProfilePictureObjectUrl = visitedUserProfile.profilePictureKey
            ? await Storage.get(visitedUserProfile.profilePictureKey, {
                level: 'protected',
          ...
          // get connections
          const graphqlConnectionsFromAuthenticatedUserToVisitedUserResult = await API.graphql(
          ...
          const inboundUserInteractionsIdIndices =
            graphqlConnectionsFromAuthenticatedUserToVisitedUserResult.data
              .queryUserInteractionsBetweenUsersByUserIdIndices
          const outboundUserInteractionsIdIndices =
            graphqlConnectionsFromVisitedUserToAuthenticatedUserResult.data
              .queryUserInteractionsBetweenUsersByUserIdIndices

          const inboundWatchingTypeConnections = inboundUserInteractionsIdIndices.filter(
          ...
          const inboundBlockingTypeConnections = inboundUserInteractionsIdIndices.filter(
