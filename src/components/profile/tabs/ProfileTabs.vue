  ...
  computed: {
    ...
    timeSpentForCanto() {
      return GetElapsedTimeBetween(
        this.userCanto.startTime,
        this.userCanto.lastUpdateTime
      )
    }
  },
  async mounted() {
    // send async queries
    const graphqlVisitedProfileCantoReq = API.graphql(
      graphqlOperation(GetCanto, { id: this.userId })
    )
    const graphqlVisitedProfileStreamsReq = API.graphql(
    ...
    const graphqlVisitedProfileBookmarkedStreamsReq = API.graphql(
      graphqlOperation(ListLikesByUser, {
        userId: this.userId,
        limit: this.fetchLimit,
        nextToken: this.bookmarkedStreamFetchNextToken
      })
    )
    const graphqlVisitedProfileOutboundInteractedUsersReq = API.graphql(
      graphqlOperation(ListUserInteractionsByActorUserIdIndex, {
        userId: this.userId,
        limit: this.fetchLimit,
    ...
    const visitedProfileStreamsDbResult =
      graphqlVisitedProfileStreamsResult.data.listStreamsByUser
    ...
    const visitedProfileBookmarkedStreamsDbResult =
      graphqlVisitedProfileBookmarkedStreamsResult.data.listLikesByUser

    const initialFetchBookmarkedStreams = visitedProfileBookmarkedStreamsDbResult.items.map(
      item => item.stream
    )
    this.bookmarkedStreamFetchNextToken =
      visitedProfileBookmarkedStreamsDbResult.nextToken
    ...
    const visitedProfileOutboundInteractedUsersDbResult =
      graphqlVisitedProfileOutboundInteractedUsersResult.data
        .listUserInteractionsByActorUserIdIndex
    ...
    this.likedUserFetchNextToken =
      visitedProfileOutboundInteractedUsersDbResult.nextToken
    this.likedUsers.push(...initialFetchLikedUsers)
    // if the result is not enough
    while (
      this.likedUsers.length < this.fetchLimit &&
      this.likedUserFetchNextToken != null
    ) {
      const rawNewFetchResult = await API.graphql(
        graphqlOperation(ListUserInteractionsByActorUserIdIndex, {
          userId: this.userId,
          limit: this.fetchLimit,
          nextToken: this.likedUserFetchNextToken
      ...
      this.likedUserFetchNextToken =
        rawNewFetchResult.data.listUserInteractionsByActorUserIdIndex.nextToken
    }
  },
  methods: {
    onToCantoClick() {
      this.$router.push({
        name: 'canto',
        params: { username: this.userCanto.user.username }
      })
    },
    ...
    async infiniteStreamTabHandler($state) {
      ...
      if (!this.userStreamFetchNextToken) {
        ...
        const scrollStreamEndNewFetchResult = await API.graphql(
          graphqlOperation(ListStreamsByUser, {
            userId: this.userId,
            limit: this.fetchLimit,
            nextToken: this.userStreamFetchNextToken
        ...
        const newPaginatedStreamByUserType =
          scrollStreamEndNewFetchResult.data.listStreamsByUser
    ...
    async infiniteBookmarkedStreamTabHandler($state) {
      ...
      if (!this.bookmarkedStreamFetchNextToken) {
        ...
        const scrollLikeEndNewFetchResult = await API.graphql(
          graphqlOperation(ListLikesByUser, {
            userId: this.userId,
            limit: this.fetchLimit,
            nextToken: this.bookmarkedStreamFetchNextToken
        ...
        const newPaginatedLikeByUserType =
          scrollLikeEndNewFetchResult.data.listLikesByUser

        this.bookmarkedStreamFetchNextToken =
          newPaginatedLikeByUserType.nextToken
        this.bookmarkedStreams.push(
          ...newPaginatedLikeByUserType.items.map(item => item.stream)
        )
    ...
    async infiniteLikedUserTabHandler($state) {
      ...
      if (!this.likedUserFetchNextToken) {
        ...
        const scrollLikedUserEndNewFetchResult = await API.graphql(
          graphqlOperation(ListUserInteractionsByActorUserIdIndex, {
            userId: this.userId,
            limit: this.fetchLimit,
            nextToken: this.likedUserFetchNextToken
        ...
        const newPaginatedLikedUserByUserType =
          scrollLikedUserEndNewFetchResult.data
            .listUserInteractionsByActorUserIdIndex
        const newPaginatedLikedUserByUserTypeFetch = newPaginatedLikedUserByUserType.items
          .filter(
            rel => rel.interactionType === this.watchTypeUserConnectionValue
          )
          .map(item => item.targetUser)
        ...
        // if the result is not enough
        while (
          newPaginatedLikedUserByUserTypeFetch < this.fetchLimit &&
          this.likedUserFetchNextToken != null
        ) {
          const rawLikedUserNewFetchResult = await API.graphql(
            graphqlOperation(ListUserInteractionsByActorUserIdIndex, {
              userId: this.userId,
              limit: this.fetchLimit,
              nextToken: this.likedUserFetchNextToken
          ...
          this.likedUserFetchNextToken =
            rawLikedUserNewFetchResult.data.listUserInteractionsByActorUserIdIndex.nextToken
        }
        $state.loaded()
      }
    }
