  ...
  >
    ...
    <v-list-item-action v-if="!blocked || showContentAnyway">
      <v-list-item-action-text>
        <v-icon v-if="showUserInteractionData" class="grey--text caption"
          >mdi-bookmark</v-icon
        >{{ bookmarkCount }}
      </v-list-item-action-text>
    </v-list-item-action>
...
export default {
  ...
  computed: {
    ...
    author() {
      ...
      return this.canto.user &&
        this.canto.user.accountStatus === this.activeUserAccountStatus
        ? this.canto.user
        : null
    },
    authorDisplayUsername() {
    ...
    timeSpentForCanto() {
      ...
      return GetElapsedTimeBetween(
        this.canto.startTime,
        this.canto.lastUpdateTime
      )
    },
    bookmarkCount() {
  ...
  methods: {
    onToCantoClick() {
      this.$router.push({
        name: 'canto',
        params: { username: this.canto.user.username }
      })
    },
