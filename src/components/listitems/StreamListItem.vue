  ...
  >
    ...
    >
      ...
      <span v-if="isSealed" class="pa-2 grey--text caption">
        <v-icon class="grey--text caption">mdi-seat-flat</v-icon
        >{{ timeFromSealedToNow }}
      </span>
      <v-spacer v-if="!isSealed" />
      <span v-if="!isSealed" class="pa-2 grey--text caption">
        <v-icon class="grey--text caption">mdi-play</v-icon>&nbsp;{{
          $t('stream.metadata.liveLabel')
        }}
      </span>
      <v-spacer />
      <span class="pa-2 grey--text caption">
        <v-icon class="grey--text caption">mdi-timer</v-icon>&nbsp;{{
          timeSpentForStream
        }}
      </span>
      <v-spacer />
      <span class="pa-2 grey--text caption">
        <v-icon class="grey--text caption">mdi-bookmark</v-icon>&nbsp;{{
          bookmarkCount
        }}
      </span>
      <v-spacer />
      <span class="pa-2 grey--text caption">
        <v-icon class="grey--text caption">mdi-comment</v-icon>&nbsp;{{
          commentCount
        }}
      </span>
      <v-spacer v-if="$vuetify.breakpoint.smAndUp" />
  ...
  >
    ...
    <v-list-item-content>
      <v-list-item-subtitle>
        <span v-if="displayUserInfo" class="body-2">{{
          authorDisplayUsername
        }}</span>
        <span v-if="displayUserInfo" class="caption">&mdash;</span>
        <span class="grey--text caption"
          ><v-icon class="grey--text caption">mdi-timer</v-icon
          >{{ timeSpentForStream }}</span
        >
        <span class="grey--text caption">,</span>
        <span v-if="isSealed" class="grey--text caption"
          ><v-icon class="grey--text caption">mdi-seat-flat</v-icon
          >{{ timeFromSealedToNow }}</span
        >
        <span v-if="!isSealed" class="red--text caption"
          ><v-icon class="red--text caption">mdi-play</v-icon>&nbsp;{{
            $t('stream.metadata.liveLabel')
          }}</span
        >
      </v-list-item-subtitle>
  ...
  computed: {
    ...
    author() {
      return this.stream.user &&
        this.stream.user.accountStatus === this.activeUserAccountStatus
        ? this.stream.user
        : null
    },
    authorDisplayUsername() {
    ...
    blocked() {
      ...
      return this.authenticatedUser.userInteractions.items.some(
        el =>
          ...
          el.targetUserId === this.author.id
      )
    },
    bookmarkCount() {
    ...
    timeFromSealedToNow() {
      return this.isSealed !== 0
        ? GetElapsedTimeTillNow(this.getNowTime, this.stream.sealTime)
        : null
    },
    timeSpentForStream() {
      return this.isSealed === 0
        ? GetElapsedTimeTillNow(this.getNowTime, this.stream.startTime)
        : GetElapsedTimeBetween(this.stream.startTime, this.stream.sealTime)
    }
  },
  methods: {
    onToStreamBtnClick() {
