<template>
  <div>
    <v-toolbar dense flat fixed extended extension-height="2">
      ...
      <v-text-field
        v-if="
          !isRouteChanging && isSearchBarVisible && $vuetify.breakpoint.mdAndUp
        "
        :placeholder="$t('common.toolbar.searchPlaceholder')"
      ...
      <the-header-authenticated-user-items
        v-if="!isRouteChanging && authenticatedUser && !isMobileSearchHeaderOn"
      />
      <!-- Unauthenticated User Items -->
      <the-header-unauthenticated-user-items
        v-else-if="
          !isRouteChanging && !authenticatedUser && !isMobileSearchHeaderOn
        "
      />
    </v-toolbar>
    <v-navigation-drawer
    ...
    >
      <v-list nav class="pt-0">
        ...
        <v-subheader v-if="$vuetify.breakpoint.smAndDown">{{
          $t('common.toolbar.menu.contentsSubheader')
        }}</v-subheader>
        <v-list-item-group
          v-model="menuContentEl"
        ...
        >
          ...
          >
            ...
            <v-list-item-content>
              <v-list-item-subtitle class="red--text text--darken-1">{{
                $t('home.bottomnav.liveNow')
              }}</v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        ...
        <v-subheader>{{
          $t('common.toolbar.menu.settingsSubheader')
        }}</v-subheader>
        <v-list-item
          v-if="authenticatedUser"
        ...
        <!-- Dark Mode Switch -->
        <v-list-item v-if="authenticatedUser">
          <v-list-item-title>{{ $t('theme.darkMode.text') }}</v-list-item-title>
          &nbsp;&nbsp;
          <v-list-item-action>
            <v-switch
              color="primary"
              v-model="authenticatedUserTheme"
            ></v-switch>
          </v-list-item-action>
        </v-list-item>
        <!-- About -->
        <v-list-item @click="onAboutClick">
  ...
  computed: {
    ...
    isHome() {
      return (
        ...
        this.getCurrentRoutePath.startsWith('/content')
      )
    },
    isSearchBarVisible() {
